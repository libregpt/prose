...
// but since we have no tools available to match and consume in the negative
// case (without regex) we need to match against our tags, then consume one char
  ...
  fn test_parse_markdown_text() {
    ...
    assert_eq!(
      ...
      Err(NomErr::Error(Error {
        ...
        code: ErrorKind::Tag
      })) /* Ok(("*but what if we italicize?", vec![MarkdownInline::Plaintext(String::from("here
           * is some plaintext "))])) */
    );
  }
